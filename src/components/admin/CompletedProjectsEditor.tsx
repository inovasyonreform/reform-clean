...
export default function CompletedProjectsEditor() {
  ...
  // Kaydet
  const saveProject = async (project: CompletedProject) => {
  ...
  const deleteProject = async (id: number) => {
    await fetch("/api/projects/completed", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-bold text-red-400">Tamamlanan Projeler Editor</h2>
      <button
        onClick={addProject}
      ...
      {projects.map((p) => (
        <div key={p.id} className="border border-neutral-700 p-4 rounded space-y-2">
          ...
          <input
            ...
            onChange={(e) =>
              setProjects(projects.map((proj) =>
                proj.id === p.id ? { ...proj, completion_year: Number(e.target.value) } : proj
              ))
            }
            className="w-full p-2 bg-neutral-900 text-white"
            placeholder="Tamamlanma Yılı"
          />
          <input
            value={p.status}
            onChange={(e) =>
          ...
          <input
            ...
            onChange={(e) =>
              setProjects(projects.map((proj) =>
                proj.id === p.id ? { ...proj, icon: e.target.value } : proj
              ))
            }
            className="w-full p-2 bg-neutral-900 text-white"
            placeholder="Icon adı (örn: BuildingOffice2Icon)"
          />
